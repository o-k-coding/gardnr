...
func newCreateDailyNoteCmd() *cobra.Command {
	...
	createDailyNoteCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			templatePath := filepath.Join(grdnr.Grdnr.RootPath, grdnr.Grdnr.TemplatePath, noteTemplate)
			dailyNote := create.CreateDailyNote(grdnr.Grdnr.RootPath, templatePath, noteDate)
